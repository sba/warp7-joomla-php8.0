...
// get menu data
require_once(JPATH_BASE.'/components/com_menus/helpers/menus.php'); //fix *** SBA ***
            ...
            <ul class="uk-list">
                <?php foreach ($menu->links as $i => $link) : ?>
                    ...
                    <?php else : ?>
                        <li>
                            <label>
                                <input value="<?php echo $link->value ?>" name="<?php echo "{$name}[{$layout}][assignment][]" ?>" type="checkbox"<?php if (isset($layouts[$layout]['assignment']) && in_array($link->value, $layouts[$layout]['assignment'])) echo ' checked="checked"' ?>> <?php echo $link->text ?>
                            </label>
                    <?php endif;  ?>
                    <?php
